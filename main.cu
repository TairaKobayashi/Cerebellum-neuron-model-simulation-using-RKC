...
int main ( int argc, char *argv [] ) 
{
  ...
  {
    double l_dt = 0.0;
    if ( t_count % 400 == 0 ) //400
    {   
      printf ( "time:%f\n", t ); 
      end = clock();
      printf ( "Now compute time = %.2f \n", ( double ) ( end - start ) / CLOCKS_PER_SEC );
    }
    ...
    if ( n_pkj > 0 ) 
    {
      ...
      while ( l_dt < H_MAX ) 
      {
        // constant input
        if ( network_sim == false ) {
          pkj_set_current <<< ( int ) ( p_pkj -> n / 128 ) + 1, 128 >>> ( d_pkj, t + l_dt );
        }
        // update
        pkj_solve_update_v ( d_pkj, d_pkj_solve, p_pkj, p_pkj_solve, d_grpkj, d_mlipkj );
    ...
    // Update synapses
    if ( network_sim == true ) {
      gr_synapse_update ( t, H_MAX, d_mfgr, d_gogr, d_go, p_gr_solve );
      go_synapse_update ( t, H_MAX, d_grgo,  d_gr, p_go_solve );
      pkj_synapse_update ( t, H_MAX, d_grpkj, d_mlipkj, d_gr, p_pkj_solve );
    }

    t_count++; t += H_MAX; 
    // record 1000msec time
    if ( t >= 500.0 && 500.0 + H_MAX > t ) { half = clock(); }
  }
  cudaProfilerStop();
