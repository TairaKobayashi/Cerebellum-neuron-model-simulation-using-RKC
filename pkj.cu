...
neuron_t *pkj_initialize ( const int nx, const int ny, const char *type, neuron_t *p_pkj )
{
  ...
  else if ( 0 == strncmp ( type, "RKC", 3 ) ) { p_pkj -> DT = H_MAX; p_pkj -> n_solver = kRKC; }
...
__global__ void pkj_set_current ( neuron_t *d_pkj, const double t )
{
  ...
  { 
    ...
    *i_ext_val = 0;  
    ...
    // fig3
    if ( ( 550 <= t && t < 1300 )  ) { *i_ext_val = PKJ_CURRENT; } //PKJ_CURRENT in pkj.cuh
  }
}
